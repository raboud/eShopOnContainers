namespace HMS.Ordering.API.Application.Commands
{
        ...
        : IRequestHandler<CreateOrderCommand, bool>
    {
        ...
        public async Task<bool> Handle(CreateOrderCommand message, CancellationToken cancellationToken)
        {
			...
			Address address = new Address(message.Street, message.City, message.State, message.Country, message.ZipCode);
			Order order = new Order(message.UserId, address, message.CardTypeId, message.CardNumber, message.CardSecurityNumber, message.CardHolderName, message.CardExpiration);
            
            foreach (CreateOrderCommand.OrderItemDTO item in message.OrderItems)
            {
